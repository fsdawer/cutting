/**
 * [After 2] 예약 생성 비동기 처리 부하 테스트
 *
 * 실행:
 *   k6 run -e STYLIST_ID=1 -e SERVICE_ID=1 k6/reservation.js
 *
 * 사전 준비:
 ...
 *
 * 검증 포인트:
 *   Before (동기): 예약 API가 rankingService + notificationService 완료까지 기다림
 *     → 응답시간 = DB 저장 + 랭킹 재계산(DB 2회) + WebSocket 전송
 *   After (비동기): 이벤트 발행만 하고 즉시 반환
 *     → 응답시간 = DB 저장만 (랭킹/알림은 별도 스레드)
 *     → 목표: p95 < 200ms
 *
...
export function setup() {
  ...
  // multiplier=10000: VU당 최대 슬롯(10000) 크기와 같아서 인접 seed 간 슬롯 겹침 없음
  // 최대 date: 2028 + 2999*10000/16/365 ≈ 7161년 (MySQL 9999년 이내 안전)
...
// ── 시나리오 설정 ─────────────────────────────────────────────────────────────
// constant-vus: 50명이 30초 동안 동시에 계속 요청 (= 50명 동시 접속 시나리오)
...
export default function (data) {
  ...
  group("예약 생성 API — @Async 비동기 처리", () => {

    // 각 VU + 반복마다 고유한 슬롯 (30분 단위, 16슬롯/일)
    // 매 테스트 실행마다 data.runSeed 값이 다르므로, 어제/방금 전 테스트와 날짜가 절대 겹치지 않음
    // runSeed * 10000: 실행 간 슬롯 범위가 완전히 분리됨 (VU당 최대 10000슬롯 블록)
    const totalSlot  = (__VU - 1) * 200 + __ITER + (data.runSeed * 10000);
    const dayOffset  = Math.floor(totalSlot / 16);
    const slotInDay  = totalSlot % 16;
    
    // BASE_DATE(2년 뒤 1월 1일)에 dayOffset을 더해서 완벽히 새로운 미래 날짜 생성
...
export function handleSummary(data) {
  ...
  const result = `
[AFTER] 예약 생성 비동기 처리  STYLIST_ID=${STYLIST_ID}  SERVICE_ID=${SERVICE_ID}
=============================================================
